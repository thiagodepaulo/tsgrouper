...
public class Teste {
	...
	public static void main(String[] args){
        ...
        DataFrame df_grouped = df.groupBy(month(col("event_start_dt")),year(col("event_start_dt"))).sum("event_amt");
        for(String s: df_grouped.columns())
        	System.out.println(s);
        df_grouped.orderBy(year(col("event_start_dt")),month(col("event_start_dt"))).show(20);
